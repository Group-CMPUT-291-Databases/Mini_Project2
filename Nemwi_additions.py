...
def list_answers(db,currentUser):
    #Print all answers related to the question
    answers_table = db["Posts"]
    test_post = answers_table.find({"Id": "64"})
    ...
    while correct_input == False:
        ...
        if input_answer >= 0 and input_answer <= len(answers_to_print)+1:
            ...
            correct_input = True
            add_vote(currentUser,postID,answerID,db,scoreNum)
...
def add_vote(currentUser,postID,answerID, db,scoreNum):
